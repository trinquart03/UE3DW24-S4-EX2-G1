...
angular.module('movieExplorerApp')
  .service('movies', function ($http) {
    ...
    {
      //on sauvegarde en mémoire le résultat
      if(!service.searchPromise)
      {
		  var requete = query;
        service.searchPromise = $http.get('https://api.themoviedb.org/3/search/movie?query='+requete+'&api_key=ededfef13bb883f988610f8f503cb5d9&language=en-US&include_adult=false').then(function(response)
        {
          service.search = response.data;
          return response.data;
        });
      }
      return service.searchPromise;
    };
